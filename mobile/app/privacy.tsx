...
const SECTIONS: { title: string; body: string }[] = [
  ...
  {
    title: '10. 계정 및 데이터 삭제 안내',
    body:
      '테니콩(Tennikong) 앱 이용자는 아래 절차로 계정 및 관련 데이터를 삭제할 수 있습니다.\n\n' +
      `1. 아래 이메일로 "계정 삭제 요청"이라는 제목과 함께 가입 시 사용한 이메일 또는 소셜 로그인 계정 정보를 보내주세요: ${CONTACT_EMAIL}\n` +
      '2. 요청을 확인한 뒤 지체 없이(영업일 기준 최대 7일 이내) 처리해 드립니다.\n\n' +
      '삭제되는 정보: 이메일 주소, 닉네임, 프로필 사진, 계정에 연결된 모든 경기 기록(날짜·상대/파트너 이름·코트·스코어·메모), 문의 및 개선 제안 내역 등 계정과 연결된 모든 개인정보\n\n' +
      '보관되는 정보: 없음. 다만 관계 법령상 보존 의무가 있는 정보는 해당 법령이 정한 기간 동안 최소한으로만 보관한 뒤 파기합니다.',
  },
];
